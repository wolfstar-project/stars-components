...
- `@wolfstar/cli` (the `stars` binary — `dev`/`build`/`info`/`codegen`/`prepare`/`commands`) is a publishable package under `packages/cli`; it only consumes the `stars.config` schema and loader owned by `@wolfstar/http-framework` (`@wolfstar/http-framework/config`), see `## The stars CLI configuration` below.

...
Stars Components is a monorepo containing **23 independently versioned TypeScript packages** under the `@wolfstar` npm scope. Its core is [`@wolfstar/http-framework`](./packages/http-framework), a framework for Discord applications that receive interactions over HTTP—well suited to webhook and serverless deployments without a persistent Gateway connection.
...
| [`@wolfstar/schema`](./packages/schema)                                             | Typed `stars.config.*` schema and loader shared by `@wolfstar/http-framework` and `@wolfstar/cli`                   | [![version](https://npmx.dev/api/registry/badge/version/@wolfstar/schema)](https://npmx.dev/package/@wolfstar/schema)                                             |
| [`@wolfstar/vite-server`](./packages/vite-server)                                   | Vite build and watch integration                                                                                    |                                                                                                                                                                   |
| [`@wolfstar/nitro-server`](./packages/nitro-server)                                 | Nitro server build integration                                                                                      |                                                                                                                                                                   |

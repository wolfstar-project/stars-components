/**
 * Remap Tolgee pull output into Shared HTTP Pieces locale directories.
 *
 * Reads .tolgee-pull/{tag}.json
 *   → packages/shared-http-pieces/src/locales/{locale}/commands/shared.json
 *
 * Tolgee short tags (en, es, pt, …) are remapped to Discord locale folder names
 * (en-US, es-ES, pt-BR, …) using `tolgeeToLocal` from `.tolgeerc.cjs`. The Tolgee
 * project stores keys in the default namespace; locally they live under
 * `commands/shared.json` (see `localNamespace` in `.tolgeerc.cjs`).
 *
...
/**
 * Tolgee language tags → local dirs under packages/shared-http-pieces/src/locales/.
 * Single source of truth: `.tolgeerc.cjs` (shared with push).
 */
const TOLGEE_TO_LOCAL: Record<string, string> = config.tolgeeToLocal;
const LOCAL_NAMESPACE = config.localNamespace ?? 'commands/shared';

const pullRoot = join(root, config.pull.path.replace(/^\.\//, ''));
const languagesRoot = join(root, 'packages/shared-http-pieces/src/locales');
...
const pullFiles = readdirSync(pullRoot, { withFileTypes: true }).filter(isJsonFile);
const pullTags = pullFiles.map((entry) => entry.name.replace(/\.json$/, ''));
...
// language files actually present in staging. Absent configured
// languages are reported but do not fail the pull.
const mappedTags = pullTags.filter((tag) => Boolean(resolveLocalDir(tag)));
if (mappedTags.length === 0) {
	console.error(`No mapped Tolgee language files found in ${pullRoot}`);
	process.exit(1);
}
...
// locales/, so an unreadable or invalid staging file cannot leave the
...
for (const tag of mappedTags) {
	...
	let content: Buffer;
	try {
		content = readFileSync(join(pullRoot, `${tag}.json`));
		JSON.parse(content.toString('utf8'));
	} catch (error) {
		console.error(`Invalid or unreadable staging file: ${tag}.json`);
		console.error(String(error));
		console.error(`Staging directory preserved for inspection: ${pullRoot}`);
		process.exit(1);
	}
	writes.push({ dest: join(languagesRoot, localDir, `${LOCAL_NAMESPACE}.json`), content });
}
...
console.log(`Remapped ${writes.length} locale files into packages/shared-http-pieces/src/locales/`);

...
export class ViteBuilder extends EventEmitter<BuilderEvents> implements Builder {
	...
	async #load(): Promise<ViteModule> {
		return this.context.importFromProject<ViteModule>(this.config.root, 'vite', INSTALL_HINT);
	}
	...
	#options(vite: ViteModule): Parameters<ViteModule['build']>[0] {
		return {
			...
			...this.config.vite,
			plugins: [this.context.pluginRegistrations(this.config), ...toArray(this.config.vite.plugins)]

...
describe('buildDevDependencies', () => {
	...
	test('GIVEN JavaScript with no linter/formatter/i18n/testing THEN produces an empty object', () => {
...
describe('buildScripts', () => {
	test('GIVEN i18n enabled THEN adds the generate:i18n script', () => {
		...
		expect(scripts['generate:i18n']).toBe('i18next-type-generator ./src/locales/en-US/ ./src/@types/i18next.d.ts');
	});
	...
	test('GIVEN JavaScript THEN start/dev run main.js directly from src', () => {
		const scripts = buildScripts(makeContext({ language: 'js' }));

		expect(scripts['start']).toBe('node src/main.js');
		expect(scripts['dev']).toBe('node --watch src/main.js');
	});

	test('GIVEN TypeScript with tsdown THEN start runs the built main.js from dist', () => {
		const scripts = buildScripts(makeContext({ language: 'ts', buildTool: 'tsdown' }));

		expect(scripts['start']).toBe('node dist/main.js');
	});

	test('GIVEN TypeScript with tsc6 THEN start still runs the built main.js from dist', () => {
		const scripts = buildScripts(makeContext({ language: 'ts', buildTool: 'tsc6' }));
...
describe('packageJson', () => {
	...
	test('GIVEN JavaScript with no linter/formatter/i18n/testing THEN devDependencies is omitted entirely', () => {

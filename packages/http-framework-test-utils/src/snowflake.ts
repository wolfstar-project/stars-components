...
export function generateSnowflake(options: SnowflakeGeneratorOptions = {}): string {
	const timestampValue = options.timestamp instanceof Date ? options.timestamp.getTime() : (options.timestamp ?? defaultTimestamp);
	...
	return String(((timestamp - discordEpoch) << 22n) | (BigInt(workerId) << 17n) | (BigInt(processId) << 12n) | BigInt(increment));
}
